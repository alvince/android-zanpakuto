package cn.alvince.zanpakuto.core.animation
...
import cn.alvince.zanpakuto.core.C.TAG
...
class AlphaTranslationBuilder internal constructor() : ViewAnimationBuilder() {
    ...
    fun changeAlpha(@FloatRange(from = 0.0, to = 1.0) fromAlpha: Float, @FloatRange(from = 0.0, to = 1.0) toAlpha: Float) {
        from = fromAlpha
        to = toAlpha
    }
}
...
class TranslationAnimationBuilder internal constructor() : ViewAnimationBuilder() {
    ...
    override val animFactory: AnimationFactory = factory {
        if (fromXType != -1 || toXType != -1 || fromYType != -1 || toYType != -1) {
            TranslateAnimation(fromXType.realType(), fromX, toXType.realType(), toX, fromYType.realType(), fromY, toYType.realType(), toY)
        } else {
